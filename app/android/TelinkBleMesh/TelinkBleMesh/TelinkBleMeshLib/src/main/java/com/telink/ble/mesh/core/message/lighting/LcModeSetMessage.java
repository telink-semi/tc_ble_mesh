 ...
 * @since 2019/8/14
 */
public class LcModeSetMessage extends GenericMessage {
    // 1 byte
    private byte mode;
     ...
     */
    public static LcModeSetMessage getSimple(int address, int appKeyIndex, byte mode, boolean ack, int rspMax) {
        LcModeSetMessage message = new LcModeSetMessage(address, appKeyIndex);
        message.mode = mode;
        message.ack = ack;
     ...
     * @param appKeyIndex        The application key index.
     */
    public LcModeSetMessage(int destinationAddress, int appKeyIndex) {
    ...
    public int getResponseOpcode() {
        return ack ? Opcode.LIGHT_LC_MODE_STATUS.value : super.getResponseOpcode();
    }
    ...
    public int getOpcode() {
        return ack ? Opcode.LIGHT_LC_MODE_SET.value : Opcode.LIGHT_LC_MODE_SET_NOACK.value;
    }
