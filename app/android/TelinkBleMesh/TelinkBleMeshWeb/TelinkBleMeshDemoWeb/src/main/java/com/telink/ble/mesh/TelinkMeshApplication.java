...
public class TelinkMeshApplication extends MeshApplication {
    ...
    /**
     * current selected mesh network
     */
    private MeshNetworkDetail meshNetworkDetail;
...
//        MeshLogger.d(meshInfo.toString());
    ...
    public void setupMesh(MeshNetworkDetail mesh) {
        MeshLogger.d("setup mesh info: " + mesh.toString());
        this.meshNetworkDetail = mesh;
        dispatchEvent(new MeshEvent(this, MeshEvent.EVENT_TYPE_MESH_RESET, "mesh reset"));
    }

    public MeshNetworkDetail getMeshInfo() {
        return meshNetworkDetail;
    }

    public List<MeshNode> getNodeList() {
        return meshNetworkDetail == null ? null : meshNetworkDetail.nodeList;
    }
    ...
    protected void onMeshEvent(MeshEvent autoConnectEvent) {
        ...
        if (MeshEvent.EVENT_TYPE_DISCONNECTED.equals(eventType)) {
            ...
            for (MeshNode nodeInfo : meshNetworkDetail.nodeList) {
    ...
    protected void onStatusNotificationEvent(StatusNotificationEvent statusNotificationEvent) {
        ...
        if (statusMessage != null) {
            MeshNode statusChangedNode = null;
            if (message.getStatusMessage() instanceof OnOffStatusMessage) {
                ...
                for (MeshNode nodeInfo : meshNetworkDetail.nodeList) {
                    if (nodeInfo.address == message.getSrc()) {
                ...
                for (MeshNode onlineDevice : meshNetworkDetail.nodeList) {
                ...
                int srcAdr = message.getSrc();
                for (MeshNode onlineDevice : meshNetworkDetail.nodeList) {
                    if (onlineDevice.address == srcAdr) {
    ...
    private boolean onLumStatus(MeshNode nodeInfo, int lum) {
    ...
    private boolean onTempStatus(MeshNode nodeInfo, int temp) {
        boolean statusChanged = false;
     ...
     */
    private void onNodeInfoStatusChanged(MeshNode nodeInfo) {
        dispatchEvent(new MeshNodeStatusChangedEvent(this, NodeStatusChangedEvent.EVENT_TYPE_NODE_STATUS_CHANGED, nodeInfo));
    }
    ...
    protected void onOnlineStatusEvent(OnlineStatusEvent onlineStatusEvent) {
        ...
        if (infoList != null && meshNetworkDetail != null) {
            MeshNode statusChangedNode = null;
            for (OnlineStatusInfo onlineStatusInfo : infoList) {
                if (onlineStatusInfo.status == null || onlineStatusInfo.status.length < 3) break;
                MeshNode deviceInfo = meshNetworkDetail.getDeviceByMeshAddress(onlineStatusInfo.address);
    ...
    protected void onNetworkInfoUpdate(NetworkInfoUpdateEvent networkInfoUpdateEvent) {
        MeshLogger.d(String.format("mesh info update from local sequenceNumber-%06X ivIndex-%08X to sequenceNumber-%06X ivIndex-%08X",
                meshNetworkDetail.provisioner.sequenceNumber, meshNetworkDetail.ivIndex,
                networkInfoUpdateEvent.getSequenceNumber(), networkInfoUpdateEvent.getIvIndex()));
        meshNetworkDetail.updateMeshSnoAndIvIndex(networkInfoUpdateEvent.getSequenceNumber(), networkInfoUpdateEvent.getIvIndex());
    }
