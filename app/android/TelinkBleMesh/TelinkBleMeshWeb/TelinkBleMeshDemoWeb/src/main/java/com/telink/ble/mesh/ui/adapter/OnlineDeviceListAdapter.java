...
public class OnlineDeviceListAdapter extends BaseRecyclerViewAdapter<OnlineDeviceListAdapter.ViewHolder> {
    List<MeshNode> mDevices;
    Context mContext;

    public OnlineDeviceListAdapter(Context context, List<MeshNode> devices) {
        mContext = context;
        mDevices = devices;
    }

    public void resetDevices(List<MeshNode> devices) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        MeshNode device = mDevices.get(position);
        ...
        if (device.address == MeshService.getInstance().getDirectConnectedNodeAddress()) {
...
//        holder.tv_name.setText(models.get(position).getAddress());
        String info;
        if (device.address <= 0xFF) {
            info = String.format("%02X", device.address);
        } else {
            info = String.format("%04X", device.address);
        }
