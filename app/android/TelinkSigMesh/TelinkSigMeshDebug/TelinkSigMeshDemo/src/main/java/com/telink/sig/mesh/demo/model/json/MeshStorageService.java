...
//        String[] groupNames = context.getResources().getStringArray(R.array.group_name);
        ...
        if (mesh.provisionerNodes == null || mesh.provisionerNodes.size() == 0) {
            // local created mesh
            ProvisionerNode provisionerNode = createNewProvisionerNode(mesh);
            meshStorage.provisioners.add(provisionerNode.provisioner);
    ...
    private ProvisionerNode createNewProvisionerNode(Mesh mesh) {
        MeshStorage.Provisioner provisioner = new MeshStorage.Provisioner();
...
//            localNode.sno = String.format("%08X", mesh.sno);
        ...
        provisionerNode.node = localNode;
        return provisionerNode;
    }
