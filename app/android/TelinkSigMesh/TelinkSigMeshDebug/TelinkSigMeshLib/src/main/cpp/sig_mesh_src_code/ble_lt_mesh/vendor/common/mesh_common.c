	...
	if(blt_sts == BLS_LINK_STATE_CONN){
		if(ble_state == BLE_STATE_BRX_E){
			if(adv_type != LL_TYPE_ADV_NONCONN_IND || 
				(my_fifo_free_cnt_get(&blt_rxfifo) <= BLE_RCV_FIFO_MAX_CNT-2)){
				return 0;
			}else{
				if(my_fifo_free_cnt_get(&blt_rxfifo) <= BLE_RCV_FIFO_MAX_CNT-2 ){
					return 0;
				}
			}
		}
		...
		if(adv_type != LL_TYPE_ADV_NONCONN_IND){
			...
			if(adv_mesh_en_flag){
				if( pAdv->data[1] != MESH_ADV_TYPE_PRO || 
					(my_fifo_free_cnt_get(&blt_rxfifo) <= 4)){	
				...
				if(my_fifo_free_cnt_get(&blt_rxfifo) <= 2){
					// special dispatch for the provision only 
					return 0;
				}
			#endif
			}else if (my_fifo_free_cnt_get(&blt_rxfifo) <= 4){
...
u8 proc_telink_mesh_to_sig_mesh(void)
{
    ...
    LOG_MSG_LIB(TL_LOG_NODE_SDK,0, 0,"sdk type 0x73000:0x%x",mesh_type);
