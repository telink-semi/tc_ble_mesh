...
void AES128_pkcs7_padding(u8 *src_buf, u32 len, u8 *padding_buf)
{
	...
	u8 padding_num = (len%AES_BLOCKLEN) ? (u8)(AES_BLOCKLEN - (len%AES_BLOCKLEN)):AES_BLOCKLEN;
	memcpy(padding_buf, src_buf, len);
	memset(padding_buf+len, padding_num, padding_num);
	return;
}
...
void AES_CBC_encrypt_buffer(AES_ctx *ctx, uint8_t* buf, uint32_t length)
{
  ...
  for (i = 0; i < length; i += AES_BLOCKLEN)
  {
    ...
    Cipher((state_t*)buf, ctx->RoundKey);
#else	
	u8 aes_result[AES_BLOCKLEN]={0};
    tn_aes_128(ctx->RoundKey, buf, aes_result);
	memcpy(buf, aes_result, AES_BLOCKLEN);
#endif
    Iv = buf;
    buf += AES_BLOCKLEN;
...
void AES_CBC_decrypt_buffer(AES_ctx* ctx, uint8_t* buf,  uint32_t length)
{
  ...
  for (i = 0; i < length; i += AES_BLOCKLEN)
  {
    ...
    InvCipher((state_t*)buf, ctx->RoundKey);
#else
    aes_decrypt(ctx->RoundKey, buf, buf);
#endif
