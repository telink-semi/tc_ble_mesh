...
- (void)updateContent:(AddDeviceModel *)model {
    NSString *addressString = model.state == AddDeviceModelStateScanned ? @"[Unallocated]" : [NSString stringWithFormat:@"0x%04X", model.scanRspModel.address];
    if (model.scanRspModel.address == 0) {
