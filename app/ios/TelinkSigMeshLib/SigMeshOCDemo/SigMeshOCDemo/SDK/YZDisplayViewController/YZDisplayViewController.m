...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        _contentScrollView.scrollEnabled = NO;//设置不可通过左右滑动切换VC
        // 注册cell
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        if (weakSelf.isUnderLineEqualTitleWidth) {
            weakSelf.underLine.yz_width = titleBounds.size.width;
        } else {
            weakSelf.underLine.yz_width = label.yz_width;
        }
        weakSelf.underLine.yz_centerX = label.yz_centerX;
    }];
