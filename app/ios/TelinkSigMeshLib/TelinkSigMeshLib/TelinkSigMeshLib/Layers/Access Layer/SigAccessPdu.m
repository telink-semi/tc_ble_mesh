...
- (BOOL)isSegmented {
    ...
    return _accessPdu.length > SigDataSource.share.defaultUnsegmentedAccessMessageLowerTransportPDUMaxLength || _message.isSegmented;
}
...
- (int)segmentsCount {
    ...
    switch (_message.security) {
        case SigMeshMessageSecurityLow:
            return 1 + (int)((_accessPdu.length + 3) / (SigDataSource.share.defaultUnsegmentedAccessMessageLowerTransportPDUMaxLength - 3));
            break;
        case SigMeshMessageSecurityHigh:
            return 1 + (int)((_accessPdu.length + 7) / (SigDataSource.share.defaultUnsegmentedAccessMessageLowerTransportPDUMaxLength - 3));
            break;
        default:
            break;
    }
