...
/// 默认一个unsegmented Access PDU的最大长度，大于该长度则需要进行segment分包，默认值为kUnsegmentedAccessMessageLowerTransportPDUMaxLength（15，如onoff：2bytes opcode + 9bytes data(1byte onoff+1byte TID+7bytes other data) + 4bytes MIC）。默认一个segmented Access PDU的最大长度为kUnsegmentedAccessMessageLowerTransportPDUMaxLength-3。
@property (nonatomic, assign) UInt16 defaultUnsegmentedAccessMessageLowerTransportPDUMaxLength;
