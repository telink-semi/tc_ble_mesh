...
- (void)sendHSLData{
    if ([self canSend]) {
        ...
        if (kControllerInHSL) {
            [DemoCommand changeHSL:YES address:address hue:self.hslModel.hue*100 saturation:self.hslModel.saturation*100 brightness:self.hslModel.lightness*100 isGet:NO respondMax:0 complete:nil];
        } else {
            [DemoCommand changeHSL:YES address:address hue:self.hsvModel.hue*100 saturation:self.hsvModel.saturation*100 brightness:self.hsvModel.value*100 isGet:NO respondMax:rsp_max complete:nil];
        }
        self.hasNextCMD = NO;
    } else {
        if (!self.hasNextCMD) {
