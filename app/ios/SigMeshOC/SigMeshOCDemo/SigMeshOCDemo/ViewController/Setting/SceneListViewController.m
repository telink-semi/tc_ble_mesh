...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell setClickRecallBlock:^{
        [DemoCommand recallSceneWithAddress:kAllDo_SIGParameters resMax:(u32)model.actionList.count sceneId:model.number complete:^(ResponseModel *m) {
            TeLog(@"recall scene %ld success",(long)model.number);
        }];
    }];
    ...
    if (array.count > 0) {
        ...
        [oprationQueue addOperationWithBlock:^{
            while (delArray.count > 0) {
                ...
                [DemoCommand delSceneWithAddress:curAction.address resMax:1 sceneId:scene.number complete:^(ResponseModel *m) {
                    TeLog(@"ResponseModel=%@",m);
