...
- (void)clickSave{
    ...
    [oprationQueue addOperationWithBlock:^{
        while (saveArray.count > 0) {
            ...
            [DemoCommand getSceneRegisterStatusWithAddress:curAction.address resMax:1 Completation:^(ResponseModel *m) {
                TeLog(@"getSceneRegisterStatusWithAddress ResponseModel=%@",m);
                TeLog(@"commondAfterNoBusy前");
                [Bluetooth.share commondAfterNoBusy:^{
                    TeLog(@"commondAfterNoBusy后");
                    [DemoCommand saveSceneWithAddress:curAction.address resMax:1 sceneId:weakSelf.model.number complete:^(ResponseModel *m) {
                        TeLog(@"saveSceneWithAddress ResponseModel=%@",m);
        ...
        while (delArray.count > 0) {
            ...
            [DemoCommand delSceneWithAddress:curAction.address resMax:1 sceneId:weakSelf.model.number complete:^(ResponseModel *m) {
                TeLog(@"delSceneWithAddress ResponseModel=%@",m);
