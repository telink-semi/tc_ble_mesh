...
- (void)normalSetting{
    [super normalSetting];
    self.title = [NSString stringWithFormat:@"%d",self.model.schedulerID];
...
- (void)clickSave{
    ...
    [oprationQueue addOperationWithBlock:^{
        while (allArray.count > 0) {
            ...
            [DemoCommand setSchedulerActionWithAddress:curDevice.address resMax:1 schedulerModel:weakSelf.model complete:^(ResponseModel *m) {
                //data字节翻转
                NSMutableData *data = [NSMutableData data];
                for (int i=15; i>=8; i--) {
                    [data appendData:[m.rspData subdataWithRange:NSMakeRange(i, 1)]];
                }
                //set scheduler success when schedulerData of send is same of schedulerData of response.
                if ([LibTools NSDataToUInt:data] == weakSelf.model.schedulerData) {
...
- (BOOL)hasInputError{
    ...
    NSArray *checkRangeMax = @[@(2099),@(31),@(23),@(59),@(59),@(kAllDo_SIGParameters)];
