...
//    self.address.text = btModel.state == DeviceStateOn ? [NSString stringWithFormat:@"%@ : %@ : %@",tempAddress,tempBrightness,tempTemperatrure]:[NSString stringWithFormat:@"%@ : 0 : 0",tempAddress];
    ...
    //直连设备显示蓝色
    if ([model.peripheralUUID isEqualToString:Bluetooth.share.store.peripheral.identifier.UUIDString] && Bluetooth.share.store.peripheral.state == CBPeripheralStateConnected) {
