...
int is_change_iv_just_before_push_cache(u8 *poll_rsp)
{
	int iv_change_flag = 0;
	mesh_cmd_bear_t *p_org_big = (mesh_cmd_bear_t *)poll_rsp; // be careful, poll_rsp is big endianness here.
	...
	if(sno_cache_little > mesh_adv_tx_cmd_sno && is_own_ele(src_addr_little)){
		...
		iv_change_flag = 1;
		//LOG_MSG_LIB(TL_LOG_FRIEND,0, 00,"change iv just before push data", 0);
	}
        ...
        if(DELAY_POLL == p_delay->delay_type){
            if(p_delay->poll_rsp){
                ...
                if(bear_tx_len <= MESH_BEAR_SIZE){
    	            //LOG_MSG_LIB(TL_LOG_FRIEND,(u8 *)&bear_temp->len, min(26,bear_temp->len+1),"enc with iv: 0x%02x%02x%02x%02x, Data for poll:", iv_idx_st.tx[0],iv_idx_st.tx[1],iv_idx_st.tx[2],iv_idx_st.tx[3]); // long log make take too mush time to response in time

					int iv_change_flag = is_change_iv_just_before_push_cache(p_delay->poll_rsp);
					if(iv_change_flag){
						mesh_decrease_ivi(iv_idx_st.tx);
					}
					...
					if(iv_change_flag){
						mesh_increase_ivi(iv_idx_st.tx); // restore
					}
