...
#define	USER_EXT_ADV_FILTER_EN					  0         // Telink mesh message use rf_pkt_aux_adv_ind_1 format. User can set 1 and add filter rules in mesh_blc_aux_adv_filter(), adv will report in app_event_handler().
    ...
    while(!(reg_rf_irq_status & FLD_RF_IRQ_RX)){		
		if (clock_time_exceed(rx_begin_tick, 8*(sizeof(rf_pkt_aux_adv_ind_1) + sizeof(mesh_cmd_bear_obfuse_t)))){ // margin = (50(wait ph before) + sizeof(dma_len)*8)us.
    		// can not compare dma len which is valid only when rx completed. 	
			 ...
			 && (pExtAdv->ext_hdr_flg == (BLE_LL_EXT_ADV_ADVA_BIT | BLE_LL_EXT_ADV_DATA_INFO_BIT))){
