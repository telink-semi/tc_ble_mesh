...
int nmw_ota_err_report(u8 errno)
{
	int ret = nmw_ctl_rsp(NMW_ERR_REPORT, &errno, sizeof(errno));
	if(nmw_ctl_rsp(NMW_ERR_REPORT, &errno, sizeof(errno))){
...
//			u8 module_sub_name[] = NMW_MODULE_SUB_DEV_NAME;
			...
			break;
		default:
			nmw_ota_err_report(CMD_UNKNOWN);
			break;
	}
...
int nmw_ota_data_save(u16 seg_index, u8 *p_data, int len)
{
...
int	nmw_ota_write(void *p)
{
	...
	nmw_ota_data_save(seg_index, ota_data, ota_len);
