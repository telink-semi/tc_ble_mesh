...
int stdio_tx(void *data, uint8_t len)
{
    ...
    if (m_stdio.conn_handle == BLE_CONN_HANDLE_INVALID || data == NULL || len > 14)
        return -1;

    errno = mi_session_encrypt(data, len, tx_buf);
    if (errno != MI_SUCCESS)
        return -2;
    else
        len += 6;                      /* 6 = 2 bytes SEQNUM + 4 bytes MIC */

    return mible_gatts_notify_or_indicate(m_stdio.conn_handle,
