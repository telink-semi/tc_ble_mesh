...
static uint32_t schd_ticks;
static uint32_t tick_interval = 10;
...
{
    uint32_t opcode = 0;
    memcpy(&opcode, pdata, MIN(len, 4));

    switch (opcode & CODE_MASK) {
    ...
    case SYS_LINK_PARAM_CONFIG:
        MI_LOG_WARNING("OPCODE 0x%08X\n", opcode);
        mi_scheduler_start(opcode);
        break;
...
{
    rxfer_timer.p_curr_tick = &schd_ticks;
    rxfer_timer.tick_interval_ms = tick_interval;
    rxfer_init(&rxfer_auth, &rxfer_timer);
    rxfer_auth.conn_handle  = conn_handle;
    rxfer_auth.srv_handle   = srv_handle;
    rxfer_auth.value_handle = auth_handle;
}
...
static PT_THREAD(update_link_layer(pt_t *pt))
{
    char pt_stat = 0;
...
#if DEBUG_MIBLE
    MI_LOG_DEBUG("update_link_layer\n");
#endif

    PT_SPAWN(pt, &pt_rxfer_tx, pt_stat = rxfer_mng_thd(&pt_rxfer_tx, &rxfer_auth, 0, 6, MAX_ATT_PAYLOAD-2));
    if (pt_stat == PT_EXITED) goto EXIT;

    PT_SPAWN(pt, &pt_rxfer_tx, pt_stat = rxfer_mng_thd(&pt_rxfer_tx, &rxfer_auth, 1, MAX_ATT_PAYLOAD-2));
...
static PT_THREAD(rxfer_test_mode(pt_t *pt))
{
    ...
    MI_LOG_DEBUG("rxfer round trip test\n");

    format_rx_cb(&rxfer_auth, buffer, sizeof(buffer));
    PT_SPAWN(pt, &pt_rxfer_rx, rxfer_rx_thd(&pt_rxfer_rx, &rxfer_auth, PASS_THROUGH, &buf_len));
    MI_LOG_INFO(MI_LOG_COLOR_BLUE"received %d bytes\n", buf_len);

    format_tx_cb(&rxfer_auth, buffer, buf_len);
    PT_SPAWN(pt, &pt_rxfer_tx, rxfer_tx_thd(&pt_rxfer_tx, &rxfer_auth, PASS_THROUGH));
    MI_LOG_INFO(MI_LOG_COLOR_BLUE"sent %d bytes\n", buf_len);
...
static int mesh_reg_auth(pt_t *pt)
{
    ...
    PT_WAIT_UNTIL(pt, (opcode_recv() & CODE_MASK) == MESH_REG_VERIFY_SUCC || (opcode_recv() & CODE_MASK) == MESH_REG_D_SIGN_INVAILD );
    if ((opcode_recv() & CODE_MASK) == MESH_REG_D_SIGN_INVAILD) {
