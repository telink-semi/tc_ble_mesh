...
int mi_mesh_otp_program()
{
    ...
    uint8_t key[16];
    keygen(key);
    mi_crypto_ccm_encrypt(key, key + 8, 8, NULL, 0, sk, 32, buffer, buffer + 32, 4);
...
int mi_mesh_otp_program_simulation()
{
		...
		if (!mi_mesh_otp_is_existed()) {
			...
			errno = mi_mesh_otp_write(OTP_MANU_CERT, get_manu_cert_pointer(), get_manu_cert_len());
			MI_ERR_CHECK(errno);
