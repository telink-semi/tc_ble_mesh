...
static struct {
    ...
    uint8_t is_sending_obj          :1;
    uint8_t is_valid_key            :1;
    uint8_t is_solicited            :1;
    uint8_t power_saving_bitmask    :2;
} flags;
...
static void mibeacon_timer_handler(void * p_context)
{
    ...
    if (dequeue(&mi_obj_queue, &obj) == MI_ERR_NOT_FOUND) {
        ...
        flags.is_sending_obj = false;

        mibeacon_adv_data_set(0, 0, m_user_data, m_user_dlen);

        if (flags.is_connected || (flags.power_saving_bitmask & 0x01)) {
            mibeacon_adv_stop();
...
#if APPEND_SCAN_RSP
        if (beacon_cfg.p_mac == NULL && !flags.is_connected) {
        ...
        uint8_t *output, uint8_t *output_len)
{
    ...
    *output++ = (uint8_t) seqnum.value;
    ...
    if (config->p_obj != NULL) {
        ...
        uint8_t mic[4];
        uint8_t aad = 0x11;
        uint32_t errno = mi_crypto_ccm_encrypt(beacon_key,
...
{
    flags.is_connected = is_connected;
    if (flags.is_sending_obj) {
        // if there is object need to send : send (non) connectable adv
        adv_start(OBJ_ADV_INTERVAL_MS, flags.is_connected);
    } else if (!flags.is_connected) {
        // if disconnect: send connectable adv
        adv_start(m_adv_interval_ms, flags.is_connected);
    }
}

int get_mibeacon_solicited_bit(void)
{
    return flags.is_solicited;
}
...
#if (HAVE_CONFIRM_BUTTON==1)
    flags.is_solicited = is_solicited;
#else
    flags.is_solicited = 1;
#endif
...
mible_status_t mibeacon_adv_start(uint16_t adv_interval_ms)
{
    ...
    return adv_start(adv_interval_ms, flags.is_connected);
}
...
#if (OBJ_QUEUE_SIZE > 0 )
int mibeacon_obj_enque(uint16_t nm, uint8_t len, void *val, uint8_t power_saving_bitmask)
{
    uint32_t errno;
    mibeacon_obj_t obj;
    ...
    flags.power_saving_bitmask = (power_saving_bitmask & 0x03);

    if (flags.is_sending_obj != true ) {
        if (flags.is_connected || (flags.power_saving_bitmask & 0x02))
            adv_start(OBJ_ADV_INTERVAL_MS, 1);
        else
            adv_start(OBJ_ADV_INTERVAL_MS, 0);
...
{
    if(0 == siid || 0 == piid || 0 == len || NULL == val || 16 == siid)
    {
        MI_LOG_ERROR("miot_spec_property_changed INVALID_PARAM error.  %s:%d\n", (uint32_t)__FILE__, __LINE__);
        return MI_ERR_INVALID_PARAM;
    }
    ...
    int ret = mibeacon_obj_enque(type, len, val, stop_adv);
    if(ret != MI_SUCCESS)
    {
        MI_LOG_ERROR("mibeacon_obj_enque error.  %s:%d\n", (uint32_t)__FILE__, __LINE__);
    }
    return ret;
}
...
{
    if(0 == siid || 0 == eiid || 16 == siid || 9 != len)
    {
        MI_LOG_ERROR("miot_spec_open_lock_event_occurred INVALID_PARAM error.  %s:%d\n", (uint32_t)__FILE__, __LINE__);
        return MI_ERR_INVALID_PARAM;
    }

    uint16_t type = 0;
    type = ((siid | 0x80) << 8) | 0x80 | eiid;
...
{
    if(0 == siid || 0 == eiid || piid_num > 2 || 16 == siid)
    {
        MI_LOG_ERROR("miot_spec_event_occurred INVALID_PARAM error.  %s:%d\n", (uint32_t)__FILE__, __LINE__);
        return MI_ERR_INVALID_PARAM;
    }
    ...
    for(int i=0; i < piid_num; i++)
    {
        memcpy(value+val_len, &(properties_array[i].piid), 1);

        val_len += 1;
            
        if(properties_array[i].len > 4)
        {
            MI_LOG_ERROR("miot_spec_event_occurred property length error.  %s:%d\n", (uint32_t)__FILE__, __LINE__);
            return MI_ERR_INVALID_PARAM;
        }
        memcpy(value+val_len, properties_array[i].val, properties_array[i].len);
        val_len += properties_array[i].len;
    }
    
    if(val_len > 9)
    {
        MI_LOG_ERROR("miot_spec_event_occurred properties total length error.  %s:%d\n", (uint32_t)__FILE__, __LINE__);
        return MI_ERR_INVALID_PARAM;
    }
        
    int ret = mibeacon_obj_enque(type, val_len, value, stop_adv);
    if(ret != MI_SUCCESS)
    {
        MI_LOG_ERROR("mibeacon_obj_enque error.  %s:%d\n", (uint32_t)__FILE__, __LINE__);
    }
