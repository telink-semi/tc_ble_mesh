...
u16 mesh_rsp_random_delay_step = 0;// 10ms unit 
...
{
	return 	((NW_BEACON_BROADCASTING == model_sig_cfg_s.sec_nw_beacon) && (0 == switch_project_flag));
}
...
_attribute_ram_code_ u8 rp_conn_adv_filter_proc(u8 * p_rf_pkt)
{
	...
	if(!memcmp(temp_uuid, p_pb_adv->uuid_pb_uuid, sizeof(temp_uuid))){
		...
		conn_adv_type_is_valid_in_extend(p_rf_pkt);
		#endif
		return 1;
	}
	return 0;
...
_USER_CAN_REDEFINE_ int ota_is_valid_pid_vid(fw_id_t *p_fw_id, int gatt_flag)
{
    ...
    if(p_fw_id->pid == fw_id_local.pid){
        u8 vid_new_1st = p_fw_id->vid & 0xFF;
        u8 vid_new_2nd = (p_fw_id->vid >> 8) & 0xFF;
							...
							u8 nk_array_idx, u8 ak_array_idx, model_common_t *pub_md, u8 immutable_flag, bear_head_t *p_tx_head)
{
	...
	}else if(mesh_need_random_delay){
		// par_type will be set to "BEAR_TX_PAR_TYPE_RSP_DELAY" in library
	}
...
int SendOpParaDebug(u16 adr_dst, u8 rsp_max, u16 op, u8 *par, int len)
{
	...
	#else
	LOG_MSG_LIB(TL_LOG_NODE_SDK,0, 0,"call send cmd API, sno:0x%x op:0x%04x",mesh_adv_tx_cmd_sno,op);
