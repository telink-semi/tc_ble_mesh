...
int cfg_cmd_send_path_echo_reply(forwarding_entry_par_t * p_fwd_entry)
{
	...
	return mesh_tx_cmd_layer_upper_ctl(CMD_CTL_PATH_ECHO_REPLY, (u8 *)&p_fwd_entry->entry.path_origin, 2, ele_adr_primary, p_fwd_entry->entry.destination, 0);
}
...
int directed_forwarding_dependents_update_start(u16 netkey_offset, u8 type, u16 path_enpoint, addr_range_big_endian_t dependent_range_addr)
{
	...
	if(p_fwd_entry){
		...
		err =  mesh_tx_cmd_layer_upper_ctl(CMD_CTL_PATH_CONFIRMATION, (u8 *)&dependent_node_update, OFFSETOF(mesh_ctl_dependent_node_update_t, dependent_addr) + (dependent_range_addr.length_present?3:2), ele_adr_primary, ADR_ALL_DIRECTED_FORWARD, 0);
	}
	#endif
	return err; 
}
